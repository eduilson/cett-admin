...
const Edit = ({user, id}: Props) => {
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const topics = (await Request(user.jwt.access_token).get('admin/question-topics?per_page=9999')).data
            setTopics(topics.data)
            const question = (await Request(user.jwt.access_token).get(`admin/questions/${id}`)).data
    ...
    const onFinish = async (values: any) => {
        ...
        try {
            (await Request(user.jwt.access_token).put(`admin/questions/${id}`, values)).data
                ...
                <Card title="Dados de cadastro" bordered={false} loading={loading}>
                    <Form.Item
                        ...
                        <Select
                            size="large"
                            showSearch
                            options={topics.map(({id, name}) => ({value: id, label: name}))}
                            filterOption={(input, option: any) =>
                                option ? option.label.toLowerCase().indexOf(input.toLowerCase()) > -1 : false
                            }/>
                    </Form.Item>
                    <Form.Item
                        label="Texto"
                    ...
                    <Form.Item
                        ...
                        <Input.TextArea rows={3} autoSize showCount/>
                    </Form.Item>
